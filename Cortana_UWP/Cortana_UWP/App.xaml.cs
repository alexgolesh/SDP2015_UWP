...
namespace Cortana_UWP
{
    ...
    sealed partial class App : Application
    {
        ...
        protected override void OnActivated(IActivatedEventArgs e)
        {
            ...
            {
                case "showSessionsInDestination":
                    string destination = speechRecognitionResult.SemanticInterpretation.Properties["destination"][0];
                    navigationParameterString = string.Format("{0}|{1}|{2}|{3}", voiceCommandName, commandMode, textSpoken, destination);
                    navigateToPageType = typeof(SessionsPage);
                    break;
        ...
        private async Task InitVCD()
        {
            var storageFile = await Windows.Storage.StorageFile.GetFileFromApplicationUriAsync(new Uri("ms-appx:///DemoCommands.xml"));
